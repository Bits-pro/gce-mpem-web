        ...
        <b-card class="auth-card" no-body>
            <div class="form-side">
                <div class="text-center">
                    <router-link to="/">
                        <span class="logo-single" style=""/>
                    </router-link>
                </div>
                <h4 class="mb-4">{{ $t('welcome') }}</h4>
                <b-form @submit.prevent="formSubmit" class="av-tooltip tooltip-label-bottom">
                    <b-form-group :label="$t('user.username')" class="has-float-label mb-4">
                        <b-form-input type="text" v-model="$v.form.email.$model" :state="!$v.form.email.$error" placeholder="Username"/>
...
export default {
    ...
    methods: {
        ...mapActions(["login", "checkAdmin", "checkSG"]),
        formSubmit() {
            this.$v.$touch();
